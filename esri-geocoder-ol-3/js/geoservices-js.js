(function(root, factory) {
	...
	/**
	 * Authenticate a user against the Geostore Service for authenticated
	 * requests.
	 * 
	 * @param {String}
	 ...
	 *            options can be null
	 * @param {Function}
	 *            callback to be called when authentication is complete
	 */
	...
	FeatureService.prototype.issueRequest = function(endPoint, parameters, cb, method) {
		...
		if (!method || method.toLowerCase() === "get") {
			...
			// TODO: change this to use method values if there are feature
			// service operations that use PUT or DELETE
	 ...
	 *            parameters
	 * @param {Function}
	 *            callback to be called when geocode is complete
	 *            geoservice.geocode({ text: "920 SW 3rd Ave, Portland, OR
	 *            97204" }, callback);
	 */
	 ...
	 *            parameters
	 * @param {Function}
	 *            callback to be called when reverse geocode is complete
	 */
	...
	function addresses(parameters, callback) {
		...
		// allow a text query like simple geocode service to return all
		// candidate addresses
		if (parameters.text) {
	...
	Batch.prototype.run = function(callback) {
		...
		if (!this.token || !this.token.token || this.token.expires < current) {
